...
func ModelForServiceResult(service *common.WrappedService, plan BackgroundWorkerModel, diags diag.Diagnostics) (*BackgroundWorkerModel, error) {
	...
	backgroundWorkerModel := &BackgroundWorkerModel{
	...
	backgroundWorkerModel.RuntimeSource = runtimeSource

	startCommand, err := common.StartCommandForEnvSpecificDetails(details.EnvSpecificDetails, runtimeSource.Runtime())
	if err != nil {
		return nil, err
	}
	backgroundWorkerModel.StartCommand = types.StringPointerValue(startCommand)

	return backgroundWorkerModel, nil
}
